...
fn solve_cubic_bezier_x_and_gradients_x(
    y: f32,
    p0: vec2<f32>,
    p1: vec2<f32>,
    p2: vec2<f32>,
    ...
    if (*_0_sol) {
        res[0].x = cubic(t.x, ax, bx, cx, dx);
        res[1].x = cubic_bezier_gradient(t.x, ax, bx, cx);
    }
    if (*_1_sol) {
        res[0].y = cubic(t.y, ax, bx, cx, dx);
        res[1].y = cubic_bezier_gradient(t.y, ax, bx, cx);
        if (res[0].x > res[0].y) {
            tmp = res[0].x;
            res[0].x = res[0].y;
            res[0].y = tmp;
    ...
    if (*_2_sol) {
        res[0].z = cubic(t.z, ax, bx, cx, dx);
        res[1].z = cubic_bezier_gradient(t.z, ax, bx, cx);
        if (res[0].x > res[0].z) {
            tmp = res[0].x;
            res[0].x = res[0].z;
            res[0].z = res[0].y;
...
fn cs_main(@builtin(num_workgroups) num_wg: vec3<u32>, @builtin(workgroup_id) wid: vec3<u32>, @builtin(local_invocation_id) liid: vec3<u32>) {
    ...
    for (var index: u32 = liid.x; index < extent.y; index = index + WORKGROUP_SIZE) {
        ...
        if (segment.typ == SEGMENT_TYPE_LINEAR) {
            ...
            let inv: f32 = sign(segment.param3.x - segment.param0.x);
            var _1: bool = false;
            var _2: bool = false;
            var _3: bool = false;
            var solution: mat2x3<f32> = solve_cubic_bezier_x_and_gradients_x(
                y,
                segment.param0,
                segment.param1,
                segment.param2,
            ...
            textureStore(output_texture, vec2<i32>(i32(index), i32(segment_index)), vec4<f32>(solution[0], y));

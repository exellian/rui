...
fn solve_cubic_bezier_y(
    ...
    if (*_0_sol) {
        res.x = cubic(t.x, ay, by, cy, dy);
    }
    if (*_1_sol) {
        res.y = cubic(t.y, ay, by, cy, dy);
        if (res.x > res.y) {
            tmp = res.x;
            res.x = res.y;
            res.y = tmp;
...
fn find_solutions_area_direction_y(
    x: f32,
    ...
    if (solution_count == 3u) {
        ...
        if (x_inv) {
            if (x > max((*segment).param0.x, (*segment).param3.x)) {
                return vec3<u32>(0u, 1u, 0u);
            }
            return vec3<u32>(1u, 0u, 0u);
        }
        if (x < min((*segment).param0.x, (*segment).param3.x)) {
            return vec3<u32>(0u, 1u, 0u);
        }
        return vec3<u32>(1u, 0u, 0u);
    }
...
fn cs_main(@builtin(num_workgroups) num_wg: vec3<u32>, @builtin(workgroup_id) wid: vec3<u32>, @builtin(local_invocation_id) liid: vec3<u32>) {
    ...
    for (var index: u32 = liid.x; index < extent.x; index = index + WORKGROUP_SIZE) {
        ...
        if (segment.typ == SEGMENT_TYPE_LINEAR) {
            ...
            var solutions: vec3<f32> = solve_cubic_bezier_y(
                x,
                segment.param0,
                segment.param1,
                segment.param2,
            ...
            let area_directions: vec3<u32> = find_solutions_area_direction_y(x, &segment, solutions, solution_count);

            // Store solution in the output texture
            textureStore(output_texture, vec2<i32>(i32(index), i32(segment_index)), vec4<f32>(solutions, x));

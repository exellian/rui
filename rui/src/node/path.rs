...
impl Builder {
    ...
    pub fn close(mut self) -> Node {
        self.segments.push(PathSegment::Linear { to: self.from });
        Node::Path(
            self.base,
            PathNode {
                from: self.from,
